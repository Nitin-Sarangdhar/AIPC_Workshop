...
def classify_image(image_path):
    ir_path = "resnet50_openvino"
    ir_model_xml = os.path.join(ir_path, "resnet50.xml")
    ir_model_bin = os.path.join(ir_path, "resnet50.bin")
    
    # Check for existing OpenVINO files first.
    if not os.path.exists(ir_model_xml) or not os.path.exists(ir_model_bin):
        print("OpenVINO files not found. Loading PyTorch model for conversion.")
        try:
        ...
        # Now convert the loaded PyTorch model
        ir_path = convert_and_save_openvino_model(model)
    else:
        print("OpenVINO files found. Skipping PyTorch model loading.")
